...
import { Menu, X, ShieldCheck, Sparkles, Globe } from 'lucide-react';
...
const Navbar: React.FC<NavbarProps> = ({ isScrolled, lang, setLang, t }) => {
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-700 border-b ${
      isScrolled 
        ? 'bg-white/95 backdrop-blur-2xl py-3 border-slate-900/10 shadow-[0_15px_40px_-15px_rgba(0,0,0,0.12)]' 
      ...
      <div className="w-full px-6 md:px-12 lg:px-16 flex justify-between items-center">
        ...
        >
          ...
          <div className="flex flex-col relative z-10">
            <span className="text-2xl md:text-4xl font-black tracking-tighter text-slate-950 serif leading-none group-hover:text-indigo-700 transition-colors">
              CAROL LIU
            </span>
            <div className="flex items-center gap-2 mt-2">
              <span className="h-0.5 w-5 bg-indigo-600 rounded-full"></span>
              <span className="text-[10px] tracking-[0.5em] text-slate-500 uppercase font-black block">
                {t.corp}
              </span>
            </div>
          </div>
        ...
        <div className="hidden lg:flex items-center space-x-12">
          <div className="flex items-center space-x-10">
            {navItems.map((item) => (
              <button
                ...
                className="text-[11px] font-black text-slate-600 hover:text-indigo-600 transition-all uppercase tracking-[0.25em] relative group"
              >
                {item.label}
          ...
          <div className="relative flex items-center bg-slate-100 rounded-full p-1.5 border-2 border-slate-200 shadow-inner group/lang">
            <div className={`absolute h-[calc(100%-12px)] w-[calc(50%-6px)] bg-indigo-600 rounded-full transition-all duration-500 ease-out ${lang === 'en' ? 'translate-x-0' : 'translate-x-full'}`}></div>
            <button 
              onClick={() => setLang('en')}
          ...
          <button
            ...
            className="group relative px-10 py-5 overflow-hidden rounded-full transition-all duration-500 active:scale-95 shadow-[0_10px_30px_-10px_rgba(79,70,229,0.3)] hover:shadow-[0_20px_40px_-10px_rgba(79,70,229,0.5)]"
          >
        ...
        <div className="flex lg:hidden items-center gap-4">
          <button 
            ...
            className="px-5 py-3 bg-indigo-600 text-white rounded-2xl font-black text-xs shadow-lg active:scale-90 transition-transform"
          >
            {lang === 'en' ? '中文' : 'EN'}
          </button>
          <button className="text-slate-950 p-3 bg-white border border-slate-200 rounded-2xl shadow-sm" onClick={() => setIsOpen(!isOpen)}>
        ...
        <div className="py-12 px-8 space-y-8">
          {navItems.map((item) => (
            <button
              key={item.id}
              onClick={(e) => handleNavClick(e, item.id)}
              className="block w-full text-left text-3xl font-black text-slate-950 hover:text-indigo-600 transition-colors"
            >
              {item.label}
            </button>
          ))}
          <div className="pt-8 border-t border-slate-100">
            <button
              onClick={(e) => handleNavClick(e, 'contact')}
              className="w-full py-7 bg-gradient-to-r from-slate-900 to-indigo-950 text-white rounded-2xl font-black uppercase tracking-[0.3em] text-xs"
            >
              {t.inquire}
            </button>
          </div>
